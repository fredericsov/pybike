...
def run():
    ...
    with tab1:
        ...
        st.markdown(f'<h1 style="color:#661d6d;font-size:24px;">{"Choississez les variables explicatives"}</h1>', unsafe_allow_html=True)
        ...
        st.markdown(f'<h1 style="color:#661d6d;font-size:24px;">{"Choix de la normalisation des données avec standard scaler"}</h1>', unsafe_allow_html=True)
        sc_value = st.radio(
                            "",
                            ('True', 'False'))
        if sc_value == 'True':
    ...
    with tab3:
        ...
        st.markdown('- Tendance générale: Les courbes valeurs réelles et valeurs prédites suivent les mêmes trajectoires.')

        predmodel = '- Les prédictions du modèle ' + df_result.iloc[0]['Model'] + ' semblent fiables'
        
        st.markdown(predmodel)
        
        st.markdown('- Nous observons cependant des décalages de quelques jours à plusieurs reprises' )

        st.markdown("- **Conclusion** : Le modèle arrive à bien déterminer des tendances par mois mais n’est pas performant sur la prévision journalière du nombre de vélo." )
